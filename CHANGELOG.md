...
#### [v3.0.0](https://github.com/darioegb/ngx-translate-routes/compare/v2.4.0...v3.0.0)

> 18 August 2026

##### Breaking Changes

- **Angular 22 minimum** — upgrade from Angular 19 to Angular 22 (`>=18.0.0` peer deps)
- **`enableSsrRouteTranslation` / `availableLanguages` removed** — these options now throw at runtime when passed to `provideNgxTranslateRoutes()`. Use the `ngx-translate-routes/ssr` entry point (coming soon).
- **`NgxTranslateRoutesModule` deprecated** — use `provideNgxTranslateRoutes()` instead. The module will be removed in a future major release.

##### New

- **`provideAppInitializer`** — `provideNgxTranslateRoutes()` now uses `provideAppInitializer()` (Angular 19+ API) instead of `APP_INITIALIZER`
- **Vitest** — test suite migrated from Karma/Jasmine to Vitest via `@angular/build:unit-test`
